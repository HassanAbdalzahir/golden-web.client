...
if (typeof document !== 'undefined') {
  const styleId = 'excel-table-styles-goldvault';
...
interface GoldVaultScreenProps {
  onBack: () => void;
}

export default function GoldVaultScreen({ onBack }: GoldVaultScreenProps) {
  ...
  const loadVaultData = async () => {
    try {
      setLoading(true);
      // Get all payments (vault operations) and statement types
  ...
  const formatAmountDisplay = (transaction: Payment) => {
    ...
    if (isBtcTransaction(transaction)) {
      // For BTC, show as pieces (assuming 1 gram = 1 piece for simplicity)
      return `${(mg / 1000).toFixed(0)} قطعة`;
    } else {
      // For gold, show as grams
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="bg-white rounded-lg shadow p-4 mb-4 flex items-center">
        ...
        <h1 className="text-2xl font-bold text-gray-800">خزنة الذهب</h1>
      </div>
          ...
          <div className="flex-1 min-h-0 relative">
            <div className="overflow-auto h-full">
              <div className="h-full relative shadow-lg border border-gray-200 rounded-lg overflow-hidden">
                <table className="w-full text-sm border-collapse h-full excel-table">
                  <thead className="bg-gradient-to-b from-blue-100 to-blue-200 sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="px-2 py-3 text-right border border-gray-300 font-semibold text-gray-700 bg-blue-50 hover:bg-blue-100 transition-colors duration-150">وزن القطعة</th>
                      <th className="px-2 py-3 text-right border border-gray-300 font-semibold text-gray-700 bg-blue-50 hover:bg-blue-100 transition-colors duration-150">الكمية بالخزنة</th>
                    </tr>
                  </thead>
                  <tbody>
                {statementTypes.map((type, index) => {
                  ...
                  // Calculate net balance in milligrams
                  ...
                  return (
                    <tr key={type.id} className="hover:bg-blue-50 transition-colors duration-150">
                      <td className="px-2 py-3 text-right border border-gray-300 font-medium">
                        {type.name}
                      </td>
                      <td className="px-2 py-3 text-right border border-gray-300">
                        {type.purity || '-'}
                      </td>
                      <td className="px-2 py-3 text-right border border-gray-300 text-gray-600">
                        {type.isBtc && type.weight ? `${type.weight} جرام` : '-'}
                      </td>
                      <td className="px-2 py-3 text-right border border-gray-300 font-mono text-blue-700 font-bold">
                        {type.isBtc ? (
                          // For BTC: Calculate units = grams / weight
                          // Guard against division by zero if weight is missing
                ...
                {statementTypes.length === 0 && (
                  <tr>
                    <td colSpan={4} className="px-2 py-8 text-center text-gray-500 border border-gray-300">
                      لا توجد بيانات
                    </td>
                  </tr>
                )}
